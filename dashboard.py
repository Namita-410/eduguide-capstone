...
def load_artifacts():
    ...
    if device == "cpu":
        # bitsandbytes' load_in_8bit needs CUDA kernels to actually speed anything up --
        # on CPU-only hardware (which is what this branch means we're on) it either does
        # nothing or fails outright. torch's own dynamic quantization is the CPU-native
        # equivalent: it quantizes the Linear layer weights (where nearly all the matmul
        # time goes) to int8 ahead of time, using only PyTorch -- no extra dependency,
        # no GPU required.
...
# Guardrail threshold on the nearest-chunk L2 distance. Calibrated from EduGuide.ipynb's
# calibration cell: in-corpus queries measured <=0.613, out-of-corpus queries measured
# >=0.881 -- 0.75 sits in that gap.
RETRIEVAL_DISTANCE_THRESHOLD = 0.75
FALLBACK_ANSWER = "I don't have documentation covering that question yet, so I can't answer it reliably."


def retrieve_context(artifacts, query, k=3):
    q_emb = artifacts["embedder"].encode([query]).astype("float32")
    distances, indices = artifacts["index"].search(q_emb, k)
    return artifacts["chunks_df"].iloc[indices[0]], distances[0]


def answer_question(artifacts, question, topic, difficulty, max_new_tokens=128):
    # k=3 and beam search restored now that quantization above claws back the CPU
    # budget these were cut to make room for -- if this is still too slow on your
    # hardware, num_beams is the first knob to turn back down (biggest cost per gain),
    # then max_new_tokens, then k, in that order.
    retrieved, distances = retrieve_context(artifacts, question, k=3)

    # If even the closest retrieved chunk is farther than the threshold, the knowledge
    # base almost certainly doesn't cover this question. Short-circuit here instead of
    # generating from irrelevant context -- this also skips the model.generate() call
    # entirely, which is the slow part on CPU.
    if distances[0] > RETRIEVAL_DISTANCE_THRESHOLD:
        return FALLBACK_ANSWER, [], retrieved.iloc[0:0]
    ...
    with torch.no_grad():
        output = model.generate(
            **inputs,
            ...
            num_beams=2,  # was 1 (plain greedy) to save time pre-quantization; 2 is a
                          # middle ground -- push toward the notebook's num_beams=4 only
                          # if your own timing on this hardware still holds up.
        )
